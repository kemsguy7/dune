...
(* This module expands either a (library ... (ctypes ...)) rule or an
   ...
   Once the above modules are provided, the ctypes field will:
...
let build_c_program
  ...
  let with_user_and_std_flags =
    let base_flags =
      ...
      let cfg = Dune_rules__Ocaml_toolchain.ocaml_config ocaml in
  ...
  let include_args =
    let open Action_builder.O in
    let* ocaml = Action_builder.of_memo ocaml in
    let ocaml_where = (Dune_rules__Ocaml_toolchain.lib_config ocaml).stdlib_dir in
