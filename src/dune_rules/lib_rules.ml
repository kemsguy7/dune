...
let build_lib
  ...
  let map_cclibs =
    cclibs (Dune_rules__Ocaml_toolchain.lib_config ocaml).ccomp_type ~flag:"-cclib"
  in
  ...
  |> Memo.Result.iter ~f:(fun compiler ->
    ...
    ; Deps
        (Foreign.Objects.build_paths
           ...
           ~ext_obj:(Dune_rules__Ocaml_toolchain.lib_config ocaml).ext_obj
           ~dir)
    ]
...
let ocamlmklib
  ...
  let build =
    let cclibs =
      Action_builder.map
        ...
        ~f:
          (cclibs
             (Dune_rules__Ocaml_toolchain.lib_config ocaml).ccomp_type
             ~flag:"-ldopt")
    in
  ...
  in
  let { Lib_config.ext_lib; ext_dll; _ } = Dune_rules__Ocaml_toolchain.lib_config ocaml in
  let dynamic_target =
    ...
    let ocamlmklib =
      let build_targets_together =
        ...
        && Dynlink_supported.get_ocaml_config
             lib.dynlink
             (Dune_rules__Ocaml_toolchain.ocaml_config (Compilation_context.ocaml cctx))
      in
      let archive_name =
...
let build_shared (lib : Library.t) ~native_archives ~sctx ~dir ~flags =
  ...
  Memo.Result.iter ocaml.ocamlopt ~f:(fun ocamlopt ->
    ...
    ; Hidden_deps
        (let ext_lib = (Dune_rules__Ocaml_toolchain.lib_config ocaml).ext_lib in
         List.rev_concat
...
let cctx (lib : Library.t) ~sctx ~source_modules ~dir ~expander ~scope ~compile_info =
  ...
  let* modes =
    let+ ocaml =
      ...
      Context.ocaml ctx
    in
    let { Lib_config.has_native; _ } = Dune_rules__Ocaml_toolchain.lib_config ocaml in
